...
/// through this trait, so that parameter must be a type implementing it. The `#[account(...)]`
/// macro implements it automatically; it is not a sealed capability boundary, so manual
/// implementations are possible but normally unnecessary.
