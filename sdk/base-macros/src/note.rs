...
fn expand_note_impl(item_impl: ItemImpl) -> TokenStream2 {
    ...
    // The account parameter type is expected to be generated by `#[account(...)]`, which also
    // emits the `ActiveAccount` impl for it; the note macro only instantiates it as the native
    // account (`Default::default()` yields the native, non-foreign binding).
...
fn parse_account_ref_type(ty: &Type) -> syn::Result<Option<(Type, bool)>> {
    let Type::Reference(type_ref) = ty else {
        return Ok(None);
    };
    // Any reference to a concrete path type other than `Word` is treated as the account
    // parameter. The referenced type is expected to be generated by `#[account(...)]`.
    ...
    if is_type_named(type_ref.elem.as_ref(), "Account") {
        return Err(syn::Error::new(
            ty.span(),
            "`crate::bindings::Account` is no longer generated; define a struct with \
             `#[account(...)]` and use that type instead",
        ));
    }
    Ok(Some(((*type_ref.elem).clone(), type_ref.mutability.is_some())))
}
...
mod tests {
    ...
    #[test]
    fn entrypoint_signature_rejects_generated_account_type() {
        let item_fn: ImplItemFn = parse_quote! {
        ...
        assert!(err.to_string().contains("no longer generated"));
    }
