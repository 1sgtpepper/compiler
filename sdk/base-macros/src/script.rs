...
pub(crate) fn expand(
    ...
    // account type is expected to be generated by `#[account(...)]`, which also emits the
    // `ActiveAccount` impl for it; here we only instantiate it as the native account
    // (`Default::default()` yields the native, non-foreign binding).
...
mod tests {
    ...
    #[test]
    fn run_signature_rejects_generated_account_type() {
        let input_fn: ItemFn = parse_quote! {
            fn run(arg: Word, account: &mut Account) {}
        };

        let err = parse_injected_param(&input_fn).unwrap_err();
